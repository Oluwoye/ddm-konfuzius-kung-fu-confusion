...
def main():
    ...
    # print(str(file_headers[j]) + " < " + str(file_headers[i]))
    # column_rdd = sc.parallelize((list_elements))
    # column_rdd.map(lambda x: (x[0], x[1:])).groupByKey().mapValues(list).collect()
    # list_elements = rdd.collect()
    # for el in list_elements:
    #    print(el)

    # with open('TPCH/tpch_customer.csv', 'r') as csv_file:
